...
var LeftToMouse : int
var RightToMouse : int
var closerToMouseX : String
...
func _physics_process(delta):
	...
	# get absolute distance from mouse to hand
	LeftToMouse = handLeft.global_position.distance_to(mousePos)
	RightToMouse = handRight.global_position.distance_to(mousePos)
	# set closer to mouse variable, reset opposite hand to normal position
	var eTween : Tween = get_tree().create_tween()
	if LeftToMouse > RightToMouse:
		eTween.tween_property(leftPoint, "position", Vector2(leftPoint.position.x, 77), 0.1)
		closerToMouseX = "right"
	elif LeftToMouse < RightToMouse:
		eTween.tween_property(rightPoint, "position", Vector2(rightPoint.position.x, 77), 0.1)
		closerToMouseX = "left"
